...
\begin{introduction}
We describe an approach to implementing a neural network in a functional programming language,
...
\textit{Back-propagation} is a common method of training artificial neural networks.
...
Conceptually, the basic building block of an artificial neural network is the neuron, 
shown in Figure \ref{fig:neuron}.
It is characterized by~\cite{gurney_introduction_1997}
\begin{itemize}
...
and no connections within a layer, so there are no loops.
...
The \textit{error} is a function of the vector difference between the output pattern 
...
This process is illustrated in Figure \ref{fig:backprop}.
...
but we introduce the \emph{synonym} \verb+ColumnVector+.
In Haskell, the \verb+type+ keyword defines an alternative name for an existing type;
it does not define a new type. (A complete code listing is available from the authors, along with a sample character recognition application.)
\begin{code}
type ColumnVector a = Matrix a
\end{code}

The final element we need to represent the neuron is the activation function.
...
We define this type using Haskell's \emph{record syntax},
and include a string to describe which activation function is being used.
...
For example, if the name of the value is \verb+s+,
...
This type is defined in the \verb+hmatrix+ package.
The activation specification, \verb+lAS+ uses the type \verb+ActivationSpec+, defined earlier.
Again we use the support for \emph{first-class functions};
...
We ensure this by not exporting the \verb+BackpropNet+ constructor outside the module,
and instead requiring the user to call a special function to construct the network.
...
Assuming that no errors are found, \verb+checkDimensions+ simply returns the second layer in a pair.

The constructor function should invoke \verb+checkDimensions+ on each pair of layers.
In an imperative language, a \emph{for loop} would typically be used.
...
simply returns the list of matrices.
The next task is to create a layer for each weight matrix supplied by the user.
In an imperative language, we might operate on each element in the weight matrix list using a \emph{for loop}.
...
The network consists of multiple layers of neurons, numbered from \begin{math}0\end{math} 
...
\begin{math}z_lk\end{math} is the output from neuron \begin{math}l\end{math} in layer \begin{math}l\end{math}.
...
The activation of neuron \begin{math}k\end{math} in layer \begin{math}l\end{math} is
...
If we're only propagated the input through the network, all we need is the output from the final layer, 
...
In traditional unit testing, the tester writes code to test individual cases.
For some applications, determining the desired result for each test case can be time-consuming,
which necessarily limits the number of cases that can be tested.
...
any code is written for the unit under test.
...
An in-depth look at QuickCheck is beyond the scope of this paper. 
...
One property it should satisfy is that if the network is trained once 
with a given input pattern and target pattern,
and immediately run on the same input pattern, the error should be reduced.
Another way of saying this is that training should reduce the error in the output layer, 
...
the complete implementation tests other properties, including the effect of training on hidden layers.)
The resulting property returns true if the error before training is less than the error after,
or if the error is already negligible (less than 0.001 for this test).
The \verb+classifyRange+ statements are useful when running the tests interactively;
they display a brief report indicating the distribution of the test coverage.
...
We can run the test in \verb+GHCi+, a Haskell interpreter.
...
that are particularly well-suited for implementing neural networks in general, 
and back-propagation in particular.
...
which can lead to a conceptual mismatch.
...
but would be wise to read the algorithm carefully in case it contains additional information
...
And some primarily functional languages provide mechanisms for doing object-oriented programming.
Crossover between these two paradigms is beneficial because it provides programmers with more ways to approach problems.
